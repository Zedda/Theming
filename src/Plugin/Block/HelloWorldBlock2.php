...
class HelloWorldBlock2 extends BlockBase {
  ...
  public function build() {
    $this->configuration['label'] = t('HWB2 - MySQL dB Query Stuff');
	// You wrap your query in the db_query function and put the results in the 
    // $result variable
	$result = db_query("select n.title, u.uid, n.created from node_field_data n, users u where u.uid = n.uid and n.type = :type order by n.created desc limit 5", array("type" => "article"));
	$content = "";
	// The resulting variable is an object with as many rows in it as there were rows 
